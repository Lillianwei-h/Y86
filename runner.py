...
def generate_answer(res, i, flag):
    ...
    # print(path2)
    # 生成答案文件所在文件名（绝对路径）
    father_file = path2 + "\\ourCode\\test_ans\\test" + str(i) + "_ans.json"       # output .json
    # father_file = path2 + "\\ourCode\\test_ans\\test" + str(i) + "_ans.yaml"     # output .yaml

    file = open(father_file, 'w')       # 打开只写文件，若无则新建
    # yaml.dump(res, file)      # output yaml
    json.dump(res, file)        # output .json
    file.close()
...
if __name__ == "__main__":
    ...
    test17 = io.StringIO(open("abs-asum-cmov.yo", "r").read())
    test18 = io.StringIO(open("abs-asum-jmp.yo", "r").read())
    test19 = io.StringIO(open("asumi.yo", "r").read())
    test20 = io.StringIO(open("j-cc.yo", "r").read())
