...
#################################################################################
# CPU模拟
# 包含寄存器对象、内存对象、条件码对象、程序状态码、以及六大阶段中使用到的任何变量
# 初始化: 寄存器、内存置零，CC初始化（ZF = True, SF= OF= False），STAT初始化（SAOK）；向内存中读入指令
# 获取CPU状态日志: 获取PC、CC、STAT、内存（8字节为单位，小端、十进制形式，非零才输出）、寄存器全部信息
#################################################################################
class CPU:
    def __init__(self):
        self.Reg = Register()
